...
firstPhase kplus1tapeStates acceptCommand otherCommands startStates multiTapeStates startFirstPhaseStates = do
    ...
    let firstPhaseFinalCommand tapeStates states sStates acc =
            case (tapeStates, states, sStates) of
                ...
                (_ : tt, s1 : ss1, s2 : ss2)    | null acc -> fpfc ES
                                                | otherwise -> fpfc LBS
                    where
...
threePhaseProcessing
    (TM
        ...
        let ([acceptCommand], otherCommands) = if null cmds1 then error "No accept command" else (cmds1, cmds2)
                where
        ...
        let startFirstPhaseStates = map genNextState multiTapeStates
        ...
        let newTMTapeAlphabets = tapeAlphabets ++ [TapeAlphabet $ Set.map BCommand commands]
